...
command.install() {
  ...
  while true; 
  do
    result=$(curl --write-out '%{response_code}' --head --silent --output /dev/null https://$GITEA_HOSTNAME/gitea/helloworld)
    if [ "$result" == "200" ]; then
	    break
    fi
    wait_seconds 5
  ...
  tmp_dir=$(mktemp -d)
  pushd $tmp_dir
  git clone https://$GITEA_HOSTNAME/gitea/helloworld 
  cd helloworld 
...
metadata:
  ...
  namespace: $cicd_prj
spec:
  url: https://$GITEA_HOSTNAME/gitea/helloworld
  git_provider:
    user: git
...
spec:
  destination:
    namespace: $dev_prj
  source:
    repoURL: https://$GITEA_HOSTNAME/gitea/helloworld-config
---
apiVersion: argoproj.io/v1alpha1
kind: Application
metadata:
...
############################################################################
  ...
  1) Go to helloworld Git repository in Gitea:
     https://$GITEA_HOSTNAME/gitea/helloworld.git
...
command.start() {
  GITEA_HOSTNAME=$(oc get route gitea -o template --template='{{.spec.host}}' -n $cicd_prj)
  info "Pushing a change to https://$GITEA_HOSTNAME/gitea/helloworld-config"
  tmp_dir=$(mktemp -d)
  pushd $tmp_dir
  git clone https://$GITEA_HOSTNAME/gitea/helloworld 
  cd helloworld 
